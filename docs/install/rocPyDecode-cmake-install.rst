.. meta::
  :description: Installing rocPyDecode and rocPyJPEGDecode using CMake
  :keywords: install, rocPyDecode, rocPyJPEGDecode, AMD, ROCm, CMake

********************************************************************
Building and installing rocPyDecode and rocPyJPEGDecode with CMake
********************************************************************
...
rocPyDecode and rocPyJPEGDecode can be built using CMake. rocPyJPEGDecode will only be built if `rocJPEG <https://rocm.docs.amd.com/projects/rocJPEG/en/latest/index.html>`_ is already installed. If rocJPEG isn't installed, rocPyJPEGDecode will be omitted and only rocPyDecode will be built and installed.

Once rocPyDecode and rocPyJPEGDecode have been built and installed, tar and deb packages can be generated for distribution. For information about generating wheel and egg files, see :doc:`pip installation instructions <rocPyDecode-pip-install>`.

The rocPyDecode and rocPyJPEGDecode source code and installation scripts are available from the `rocPyDecode GitHub Repository <https://github.com/ROCm/rocPyDecode>`_. 
...
Before building and installing rocPyDecode and rocPyJPEGDecode, run `rocPyDecode-requirements.py <https://github.com/ROCm/rocPyDecode/blob/develop/rocPyDecode-requirements.py>`_ from the ``rocPyDecode`` root directory:
...
To build rocPyDecode and rocPyJPEGDecode, create a ``build`` directory in the ``rocPyDecode`` root directory:
...
Once the makefile has been generated, make and install rocPyDecode and rocPyJPEGDecode:
...
.. note::
  ...
  The deb files and tar files include rocPyJPEGDecode.
