...
assets present, expect **seven tests**: binding types, raw H.264, raw H.265,
batched JPEG decoding, video/JPEG regression checks, and the configure regression
suite. The configure suite also runs when video is skipped or disabled, provided
a Python 3 interpreter is available. Set ``-DBUILD_TESTING=OFF`` to omit test
registration.
