...
def Decoder(
    ...
    # decoder instance
    viddec = dec.decoder(
        device_id, mem_type, codec_id, b_force_zero_latency, crop_rect, 0, 0, 1000
    )
    ...
    viddec.SetReconfigParams(
        flush_mode, output_file_path if (output_file_path is not None) else str("")
    )
            ...
            if output_file_path is not None:
                if frame_is_resized:
                    ...
                    viddec.SaveFrameToFile(
                        output_file_path,
                        packet.frame_adrs_resized,
                        resized_surface_info,
                    )
                else:
                    surface_info = viddec.GetOutputSurfaceInfo()
                    viddec.SaveFrameToFile(
    ...
    if output_file_path is None:
        if n_frame > 0 and total_dec_time > 0:
            ...
            print(
                "info: avg decoding time per frame: "
                + "{0:0.2f}".format(round(time_per_frame, 2))
                + " ms"
            )
            print(
                "info: avg frame per second: "
                + "{0:0.2f}".format(round(frame_per_second, 2))
                + "\n"
            )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input", type=str, help="Input File Path - required", required=True
    )
    parser.add_argument(
        "-o", "--output", type=str, help="Output File Path - optional", required=False
    )
    parser.add_argument(
        "-d",
        "--device",
