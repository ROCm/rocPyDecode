...
Compiler warnings are enabled by default for component sources (`-Wall` with Clang/GCC).
Use `-DROCPYDECODE_ENABLE_WARNINGS=OFF` at configure time to disable this setting.
Installed SDK headers are treated as system headers; warnings from SDK utility
source files compiled into the video module are suppressed separately.
